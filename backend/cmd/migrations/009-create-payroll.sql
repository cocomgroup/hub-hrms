...
-- Payroll Periods Table (pay cycles)
...
-- Pay Stubs Table (main payroll records)
--DROP TABLE pay_stubs;
CREATE TABLE IF NOT EXISTS pay_stubs (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'approved', 'paid', 'voided')),
    paid_date DATE,
    check_number VARCHAR(50),
    payment_method VARCHAR(20) CHECK (payment_method IN ('check', 'direct_deposit', 'wire', 'cash')),
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_pay_stubs_period ON pay_stubs(payroll_period_id);
...
--DROP TABLE pay_stub_deductions;
...
DROP TRIGGER IF EXISTS update_pay_stubs_updated_at ON pay_stubs;
