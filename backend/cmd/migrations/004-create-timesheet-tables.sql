...
CREATE TABLE IF NOT EXISTS time_entries (
    ...
    notes TEXT,
    timesheet_id UUID, -- References timesheets table (created below)
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    CONSTRAINT unique_employee_date UNIQUE(employee_id, entry_date)
);

-- Create simplified timesheets table (weekly submissions)
CREATE TABLE IF NOT EXISTS timesheets (
    ...
    CONSTRAINT fk_timesheet_id FOREIGN KEY (timesheet_id) REFERENCES timesheets(id) ON DELETE SET NULL
);
