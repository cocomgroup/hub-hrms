	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(authSvc *MockAuthService) {
				...
				authSvc.On("ValidateToken", "valid.token.here").Return(validToken, nil)
			},
			expectedStatus: http.StatusOK,
			checkContext:   true,
		},
		...
		},
		{
			...
			setupMocks: func(authSvc *MockAuthService) {
				...
				authSvc.On("ValidateToken", "expired.token.here").Return(invalidToken, nil)
			},
			expectedStatus: http.StatusUnauthorized,
			checkContext:   false,
		},
...
func BenchmarkAuthMiddleware(b *testing.B) {
	...
	mockAuthSvc.On("ValidateToken", tokenString).Return(validToken, nil)
